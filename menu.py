...
class PluginCard(CardWidget):  # 插件卡片
    def __init__(
        ...
        if self.url:
            menu_actions.append(
                Action(fIcon.LINK, f"访问“{title}”插件页面", triggered=lambda: QDesktopServices.openUrl(QUrl(self.url)))
            )
        menu_actions.append(Action(fIcon.DELETE, f"卸载“{title}”插件", triggered=self.remove_plugin))
    ...
    def run(self) -> None:
        try:
            ...
            audio_file = generate_speech_sync(
                text=self.text, engine=self.engine, voice=self.voice, auto_fallback=True, timeout=10.0
            )
...
class SettingsMenu(FluentWindow):
    ...
    def _import_from_plugin_json(self, json_file_path):
        try:
            ...
            if os.path.exists(target_dir):
                reply = MessageBox("插件已存在", f'插件 "{plugin_name}" 已存在, 是否覆盖？', self).exec_()
                if reply != MessageBox.Yes:
                    return
                shutil.rmtree(target_dir)
            shutil.copytree(source_dir, target_dir)
            self.refresh_plugin_list()
            w = MessageBox("导入成功", f'插件 "{plugin_name}" 导入成功！\n重启应用后生效。', self)
    ...
    def setup_help_interface(self):
        ...
        open_by_browser.clicked.connect(
            lambda: QDesktopServices.openUrl(QUrl("https://classwidgets.rinlit.cn/docs-user/"))
        )
            ...
            [
                ...
                Action(
                    ...
                    triggered=lambda: tip_toast.push_notification(
                        4, title="通知", subtitle="测试通知示例", content="这是一条测试通知ヾ(≧▽≦*)o"
                    ),
                ),
            ]
        )
    ...
    def available_voices_cnt(self, voices):
        ...
        if (
            ...
            and hasattr(self, "TTSSettingsDialog")
            and self.TTSSettingsDialog
            and not self.TTSSettingsDialog.isHidden()
        ):
    ...
    class TTSSettings(MessageBoxBase):  # TTS设置页
        ...
        def play_tts_preview(self, text_type):
            ...
            format_values = defaultdict(
                str,
                {"lesson_name": "信息技术", "minutes": "5", "title": "通知", "content": "这是一条测试通知ヾ(≧▽≦*)o"},
            )
            ...
            try:
                ...
                MessageBox("TTS预览失败", f"启动TTS预览时出错: {str(e)}", self).exec()
    ...
    def load_tts_voices_for_engine(self, engine_key):
        ...
        self.tts_voice_loader_thread.voicesLoaded.connect(
            lambda voices: self.available_voices_cnt(voices) or self.switch_enable_TTS.setEnabled(True)
        )
        self.tts_voice_loader_thread.errorOccurred.connect(
            lambda error: self.handle_tts_load_error(error) or self.switch_enable_TTS.setEnabled(True)
        )
    ...
    def toggle_tts_settings(self, checked):
        ...
        if checked:
            ...
            if (
                self.voice_selector.itemText(0) in ["未启用", "加载失败", "无可用语音"]
    ...
    def update_tts_voices(self, available_voices):
        ...
        switch_enable_TTS.setEnabled(True)
        voice_selector.currentTextChanged.connect(
            lambda name: config_center.write_conf("TTS", "voice_id", voice_selector.currentData())
            if voice_selector.currentData()
            else None
        )
    ...
    def setup_customization_interface(self):
        ...
        select_weather_api.setCurrentIndex(
            weather.weather_manager.api_config["weather_api_list"].index(config_center.read_conf("Weather", "api"))
        )
        select_weather_api.currentIndexChanged.connect(
            lambda: config_center.write_conf(
                "Weather",
                "api",
                weather.weather_manager.api_config["weather_api_list"][select_weather_api.currentIndex()],
            )
        )
    ...
    def setup_about_interface(self):
        ...
        github_page.clicked.connect(
            lambda: QDesktopServices.openUrl(QUrl("https://github.com/RinLit-233-shiroko/Class-Widgets"))
        )
        ...
        thanks_button.clicked.connect(
            lambda: QDesktopServices.openUrl(
                QUrl("https://github.com/RinLit-233-shiroko/Class-Widgets?tab=readme-ov-file#致谢")
            )
        )
            ...
            if config_center.read_conf("Time", "type") == "ntp":
                try:
                    self.show_info_toast(
                        "时区设置", f"时区已更新为 {ntp_sync_timezone.currentText()}, 正在重新同步时间~"
                    )
    ...
    def _process_ntp_url_change(self, url: str, ntp_server_url_widget):
        ...
        if not url:
            self._show_ntp_flyout(
                ntp_server_url_widget, "warning", "NTP服务器URL不能为空 o(〃＾▽＾〃)o\n请输入有效的NTP服务器地址", ""
            )
            return
        # 验证url的正则
        # 筛选demo:
        ...
        if pattern.match(url):
            ...
            self._show_ntp_flyout(ntp_server_url_widget, "success", f"NTP服务器已更新: {url}")
                    ...
                    if self.parent_menu and hasattr(self.parent_menu, "adInterface"):
                        ...
                        if ntp_url_widget:
                            ...
                            self.parent_menu._show_ntp_flyout(
                                ntp_url_widget, "success", f"NTP服务器已更新: {suggestion}"
                            )
                ...
                try:
                    if self.parent_menu:
                        ...
                        if hasattr(self.parent_menu, "_show_ntp_flyout"):
                            self.parent_menu._show_ntp_flyout(
                                ntp_url_widget if ntp_url_widget else None, "success", "设置已保存ヾ(≧▽≦*)o"
                            )
                except Exception as e:
    ...
    def setup_timeline_edit(self):  # 底层大改
        ...
        te_part_time.timeChanged.connect(lambda: self.show_tip_flyout("重要提示", "请使用 24 小时制", te_part_time))
    ...
    def cf_export_schedule(self, file_name):  # 导出课程表
        file_path, _ = QFileDialog.getSaveFileName(self, "保存文件", file_name, "Json 配置文件 (*.json)")
        if file_path:
    ...
    def cf_import_schedule(self):  # 导入课程表
        ...
        if file_path:
            ...
            if list_.import_schedule(file_path, file_name):
                ...
                self.show_tip_flyout(
                    "您已成功导入课程表配置文件",
                    f"文件将导入于{file_name}, 请手动切换您的配置文件。",
                    self.import_from_file,
                    InfoBarIcon.SUCCESS,
                    FlyoutAnimationType.PULL_UP,
                )
    ...
    def cf_receive_schedule(self, data):
        if not (data.get("error", None) is None):
            self.show_tip_flyout(
                "获取配置文件失败", data["error"], self.config_download, InfoBarIcon.ERROR, FlyoutAnimationType.PULL_UP
            )
            return
        try:
            ...
            self.show_tip_flyout(
                "更新配置文件失败", f"{e}", self.config_download, InfoBarIcon.ERROR, FlyoutAnimationType.PULL_UP
            )
            return
        schedule_center.update_schedule()
    ...
    def cf_post_schedule(self):
        ...
        try:
            ...
            self.show_tip_flyout(
                "上传配置文件失败", f"{e}", self.config_upload, InfoBarIcon.ERROR, FlyoutAnimationType.PULL_UP
            )

    def cf_receive_schedule_from_post(self, data):
        if data.get("error", None):
            self.show_tip_flyout(
                "上传配置文件失败", data["error"], self.config_upload, InfoBarIcon.ERROR, FlyoutAnimationType.PULL_UP
            )
    ...
    def te_delete_part(self):
        alert = MessageBox(
            "您确定要删除这个时段吗？", "删除该节点后, 将一并删除该节点下所有课程安排, 且无法恢复。", self
        )
