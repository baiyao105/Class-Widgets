...
from conf import base_directory  # type: ignore
from file import config_center


def cache_result(expire_seconds: int = 300) -> Any:
    """缓存装饰器"""
...
class WeatherManager:
    ...
    def fetch_weather_data(self) -> Dict[str, Any]:
        ...
        try:
            ...
            result = {"now": weather_data, "alert": alert_data or {}}
...
class WeatherDataProcessor:
    ...
    def _load_weather_status(self, api_name: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            with open(
                os.path.join(base_directory, "config", "data", f"{api_name}_status.json"), "r", encoding="utf-8"
            ) as f:
    ...
    def _normalize_xiaomi_alert(self, alert: Dict[str, Any]) -> Dict[str, Any]:
        ...
        display_text = (
            f"{alert_type}{alert_level}预警"
            if alert_type and alert_level
            else f"{alert_type}预警"
            if alert_type
            else "天气预警"
        )
        return {
