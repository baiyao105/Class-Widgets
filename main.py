...
from utils import DarkModeWatcher, TimeManagerFactory, restart, share, stop, update_timer
...
def get_timeline_data() -> Dict[str, Any]:
    if len(loaded_data["timeline"]) == 1:
        ...
        if (
            str(current_week) in loaded_data["timeline"] and loaded_data["timeline"][str(current_week)]
        ):  # 如果此周有时间线
...
def get_hide_status() -> int:
    ...
    return (
        ...
        }[config_center.read_conf("General", "hide")]()
        and not (current_lesson_name in excluded_lessons)
        else 0
    )
...
class PluginMethod:  # 插件方法
    ...
    def generate_speech(
        ...
        str: 生成的音频文件路径
        """
        return generate_speech_sync(text=text, engine=engine, voice=voice, auto_fallback=auto_fallback, timeout=timeout)
...
class DesktopWidget(QWidget):  # 主要小组件
    ...
    def init_ui(self, path: str) -> None:
        ...
        self.setAttribute(
            Qt.WidgetAttribute.WA_TransparentForMouseEvents, config_center.read_conf("General", "enable_click") == "0"
        )
    ...
    def update_data(self, path: str = "") -> None:
        ...
        if path == "widget-current-activity.ui":  # 当前活动
            ...
            if (
                isDarkTheme()
                and theme_config.support_dark_mode
                or isDarkTheme()
                and theme_config.default_theme == "dark"
            ):  # 在暗色模式显示亮色图标
    ...
    def toggle_weather_alert(self) -> None:
        ...
        if self.showing_temperature:
            ...
            if (
                hasattr(self, "current_alerts")
                and self.current_alerts
                and len(self.current_alerts) > 1
                and hasattr(self, "current_alert_index")
            ):
    ...
    def _get_alert_icon_by_severity(self, severity: Union[str, int]) -> str:
        """根据预警等级获取对应图标"""
        severity_color_map = {"1": "blue", "2": "yellow", "3": "orange", "4": "red"}
    ...
    def update_weather_data(self, weather_data: Dict[str, Any]) -> None:  # 更新天气数据(已兼容多api)
        ...
        if type(weather_data) is dict and hasattr(self, "weather_icon") and "error" not in weather_data:
            ...
            try:
                ...
                current_city.setText(f"{db.search_by_num(config_center.read_conf('Weather', 'city'))} · {weather_name}")
        ...
        if config_center.read_conf("Other", "initialstartup") == "1":  # 首次启动
            try:
                ...
                conf.add_shortcut_to_startmenu(
                    f"{base_directory}/ClassWidgets.exe", f"{base_directory}/img/favicon.ico"
                )
